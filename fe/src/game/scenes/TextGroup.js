...
export default class TextGroup extends Phaser.GameObjects.Group {
    ...
    showTextFailHide(scene, x, y) {
        const text = scene.add.text(x,y,"숨을 수 없습니다!",this.styles.style1);
        this.add(text);
        ...
        showTextFind(scene, x, y) { //플레이어 화면의 중간에 띄울 예정 
    ...
    showTextFailFind(scene, x, y) { // 항상 플레이어 화면의 중간에 띄울예정
        const text = scene.add.text(x,y, "찾기에 실패했습니다!", this.styles.style2);
        this.add(text);
